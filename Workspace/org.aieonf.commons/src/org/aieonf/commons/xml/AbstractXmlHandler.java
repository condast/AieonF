...
//
public abstract class AbstractXmlHandler<T extends Object, E extends Enum<E>> extends DefaultHandler implements IXMLHandler<T>{
	...
	protected AbstractXmlHandler( EnumSet<E> enumSet ) {
		this( AbstractXmlHandler.class, enumSet );
	}

	protected AbstractXmlHandler( Class<?> clss, EnumSet<E> enumSet ) {
		this.clss = clss;
		this.nodes = enumSet;
	 ...
	 * @param node
	 */
	protected abstract void completeNode( Enum<E> node );
	 ...
	 * @param value
	 */
	protected abstract void addValue( Enum<E> node, String value );
	...
	public void characters(char ch[], int start, int length) throws SAXException {
		...
		if( StringUtils.isEmpty( value ))
			return;
		Enum<E> current = stored.lastElement().key;
	...
	private E getNode( String name ){
		for( E enm: nodes ){
			if( enm.name().equals( name ))
				return enm;
		}
		return null;
	}
	...
	protected static class Node<E extends Enum<E>, T extends Object>{
		private String id;
		...
		Node( String id, Enum<E> key, T data, Attributes attributes) {
			this( id, null, key, data, attributes );
		}

		Node( String id, Node<E, T> parent, Enum<E> key, T data, Attributes attributes) {
			super();
			this.id = id;
			this.parent = parent;
			this.key = key;
		...
		public String getAttribute( Enum<?> enm ) {
			return AbstractXmlHandler.getAttribute( this.attributes, enm );
		}
	}
