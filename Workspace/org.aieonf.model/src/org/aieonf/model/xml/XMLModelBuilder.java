...
public class XMLModelBuilder<T extends IDescriptor> implements IModelBuilder<T> {
	 ...
	 */
	public void addListener( IModelBuilderListener<IModelLeaf<IDescriptor>> listener ){
	 ...
	 */
	public void removeListener( IModelBuilderListener<IModelLeaf<IDescriptor>> listener ){
	...
	public IModelLeaf<T> build() {
		...
		try {
			...
			logger.severe( S_ERR_NO_TEMPLATE_FOUND + this.builder.getURL() + "\n");
			e1.printStackTrace();
			return null;
		}
		...
		try {
			...
			IModelBuilderListener<IModelLeaf<IDescriptor>> listener = new IModelBuilderListener<IModelLeaf<IDescriptor>>(){

				@Override
				public void notifyChange(ModelBuilderEvent<IModelLeaf<IDescriptor>> event) {
					for( IModelBuilderListener<IModelLeaf<IDescriptor>> listener: listeners )
