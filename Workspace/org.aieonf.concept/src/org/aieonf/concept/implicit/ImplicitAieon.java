...
public class ImplicitAieon extends ConceptWrapper implements IImplicitAieon<IDescriptor>
{
  ...
  */
  ...
  /**
   * This utility checks to see if the given descriptor has children, and
   * if so checks if there is a child that is implicit to the given implicit aieon 
   * @param implicit
   * @param descriptor
   * @return
   */
  public static final boolean childImplies( IImplicitAieon<IDescriptor> implicit, IDescriptor descriptor )
  	throws ConceptException
  {
  	...
  	for( IRelationship relationship: relationships ){
  		if( EmbeddedRelationship.isEmbedded( relationship )){
  			if( implicit.implies( relationship.getConceptDescriptor() ))
  				return true;
  			if( implicit.accept( relationship.getConceptDescriptor() ))
  				return true;
  		}
  	}
  	return false;				
