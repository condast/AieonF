...
public class TemplateInterpreter extends AbstractModelInterpreter<IDescriptor, IDescriptor> {
	...
	public TemplateInterpreter( Class<?> clss, InputStream in )  {
		super( IModelBuilder.S_DEFAULT_LOCATION, in );
		this.clss = clss;
	}

	public TemplateInterpreter( Class<?> clss, String location ) throws IOException {
	...
	public synchronized IDescriptor onCreate( String name, Attributes attributes) {
		...
		descriptor = new ODescriptor( name_attr );
		switch( dc ){
		case CONTEXT:
			this.context = new ContextAieon( descriptor);
			if(!Utils.assertNull( name_attr))
				this.context.set( IContextAieon.Attributes.CONTEXT, name_attr);
			descriptor = this.context;
			break;
		case DOMAIN:
			descriptor = new DomainAieon( descriptor);
			break;
		case LOADER:
			...
			break;
		case CONCEPT:
			descriptor = createConcept( OConceptBase.class, attributes );
			break;			
		case CATEGORY:
			descriptor = new CategoryAieon( descriptor );
			if(!Utils.assertNull( name_attr))
				this.context.set( CategoryAieon.Attributes.CATEGORY, name_attr);
			break;
		case URL:
			descriptor = new URLAieon( descriptor);
			break;			
		case LOG_ENTRY:
			descriptor = new LogEntryAieon( descriptor );
			break;			
		case BIRTH_DATE:
			descriptor = new DateAieon( descriptor );
			break;
		case LOCALE:
			descriptor = new LocaleAieon( descriptor );
			if(!Utils.assertNull( name_attr))
